...
public abstract class AbstractArcheologyWorkbenchScreen<T extends AbstractArcheologyWorkbenchScreenHandler> extends HandledScreen<T> implements RecipeBookProvider {
    private static final Identifier RECIPE_BUTTON_TEXTURE = new Identifier("textures/gui/recipe_button.png");
    public final AbstractFurnaceRecipeBookScreen recipeBook;
    private boolean narrow;
    private final Identifier background;

    public AbstractArcheologyWorkbenchScreen(T handler, AbstractFurnaceRecipeBookScreen recipeBook, PlayerInventory inventory, Text title, Identifier background) {
    ...
    public void init() {
        ...
        this.titleX = (this.backgroundWidth - this.textRenderer.getWidth(this.title) - 4);
    }
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        int l;
        if (((AbstractArcheologyWorkbenchScreenHandler)this.handler).isBurning()) {
            l = ((AbstractArcheologyWorkbenchScreenHandler) this.handler).getFuelProgress();
                this.drawTexture(matrices, i + 82, j + 35 + 14 - l, 176, 13 - l, 14, l + 1);
        }
        l = ((AbstractArcheologyWorkbenchScreenHandler)this.handler).getCookProgress();
        this.drawTexture(matrices, i + 79, j + 19, 177, 16, l+1, 14);
    }
