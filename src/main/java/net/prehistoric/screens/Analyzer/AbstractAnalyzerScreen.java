...
public abstract class AbstractAnalyzerScreen<T extends AbstractAnalyzerScreenHandler> extends HandledScreen<T> implements RecipeBookProvider {
    private static final Identifier RECIPE_BUTTON_TEXTURE = new Identifier("textures/gui/recipe_button.png");
    public final AbstractFurnaceRecipeBookScreen recipeBook;
    private boolean narrow;
    private final Identifier background;

    public AbstractAnalyzerScreen(T handler, AbstractFurnaceRecipeBookScreen recipeBook, PlayerInventory inventory, Text title, Identifier background) {
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        int l;
        if (((AbstractAnalyzerScreenHandler)this.handler).isBurning()) {
            l = ((AbstractAnalyzerScreenHandler) this.handler).getFuelProgress();
            this.drawTexture(matrices, 0, 0, 0, 0, 0, 0);
        }
        l = ((AbstractAnalyzerScreenHandler)this.handler).getCookProgress();
        this.drawTexture(matrices, i + 69, j + 29, 177, 18, l+1, 9);
    }
