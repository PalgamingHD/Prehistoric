...
public abstract class AbstractSifterScreen<T extends AbstractSifterScreenHandler> extends HandledScreen<T> implements RecipeBookProvider {
    private static final Identifier RECIPE_BUTTON_TEXTURE = new Identifier("textures/gui/recipe_button.png");
    public final AbstractFurnaceRecipeBookScreen recipeBook;
    private boolean narrow;
    private final Identifier background;

    public AbstractSifterScreen(T handler, AbstractFurnaceRecipeBookScreen recipeBook, PlayerInventory inventory, Text title, Identifier background) {
    ...
    protected void drawBackground(MatrixStack matrices, float delta, int mouseX, int mouseY) {
        ...
        int l;
        if (((AbstractSifterScreenHandler)this.handler).isBurning()) {
            l = ((AbstractSifterScreenHandler)this.handler).getFuelProgress();
            this.drawTexture(matrices, 0, 0, 0, 0, 0,  0);
        }

        l = ((AbstractSifterScreenHandler)this.handler).getCookProgress();
        this.drawTexture(matrices, i + 75, j + 33, 181, 2, 26, l + 1);
    }
